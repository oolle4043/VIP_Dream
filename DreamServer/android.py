...
@router.post("/alarm_status/{user_id}/{alarm_status}/",status_code=200)
async def alarm_status(user_id:str, alarm_status: bool, db:Session = Depends(get_db)):
    """
    알람 On, Off API
    """
    exist_user_id = db.query(models.User.id).filter_by(user_id = user_id).scalar_subquery()
    alarm = db.query(models.Device.alarm_status).filter_by(device_user_id = exist_user_id).first()
 
    if not exist_user_id:
        return {"result":"FALSE"}, 401
    
    db.query(models.Device).filter(models.Device.device_user_id == exist_user_id).update({"alarm_status":alarm_status})
    db.commit()
    return {"result":"200"}, 200